''' Implementation of moving stars net. '''
...
class Window():
    ...
    def _update_stars(self) -> None:
        ...
        if len(self.stars) < stars_per_row*stars_per_col:
            for col in range(stars_per_col):
                new_star: Star = Star()
                new_star.rect.x += star_width + 4*star_width*col  # type: ignore
                new_star.rect.y -= star_height  # type: ignore
